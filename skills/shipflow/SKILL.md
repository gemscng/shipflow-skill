...
| "open an issue about X" / "file an issue" | `renaiss-shipflow issue create --title "X" --body "..."` — body per the issue-body ladder (`references/loop-mode.md` § "Message style"); for anything visible (broken layout, wrong render, error dialog) add `--screenshot <path...>` (+ `--screenshot-caption` per shot): hosted and embedded in the body, worth more than prose |
