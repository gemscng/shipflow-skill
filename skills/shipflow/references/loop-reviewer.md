...
| **A second, narrower door exists** | a numbered `N: answer` decision reply can also release it, under four preconditions — read them in `loop-mode.md` § `needs-reporter-review` (→ `contracts/shipflow-contract.json` → `intentGate.$comment`), not from a copy here. You use neither door |
...
issue-body ladder — in `loop-mode.md` § "Message style". Don't restate it
here.
