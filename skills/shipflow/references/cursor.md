...
|---|---|
| `AskUserQuestion` tool | Cursor's native AskQuestion tool — direct mapping, same option/label shape. Headless (`cursor-agent -p` / scheduled ticks): NEVER wait — proceed per policy or `renaiss-shipflow issue escalate <n> --reason "..."`; set `SHIPFLOW_HEADLESS=1` |
...
| `CronCreate` continuous loop | No in-session scheduler: loop in-session — after each pass `sleep` the interval in ≤300 s chunks (raise the shell tool timeout per chunk), then the next pass, until the user interrupts or says `stop`; never stop after one pass. `once` = one pass. Always-on: an EXTERNAL scheduler (cron/launchd/CI) re-invokes one `once` pass per tick — headless `cursor-agent -p "$(cat <skill root>/commands/shipflow-loop.md) once"`. One loop per gh identity still applies |
