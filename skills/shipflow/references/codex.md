...
| Skill tool → `shipflow:smart-commit` (every loop commit) | No plugin namespace: read and follow the skill file from the plugin clone — `~/.shipflow-skill/skills/smart-commit/SKILL.md` — and execute its plan (same loop adaptations: no AI-attribution trailer, skip the human-confirm gate). Never a bare `smart-commit`, never a repo-relative path |
