...
var init_shipflow_contract_data = __esm(() => {
  SHIPFLOW_CONTRACT = {
    ...
    markers: {
      ...
      intakeTrusted: "<!-- shipflow:intake-trusted",
...
function parseIntakeTrustedAttestation(c, trustedSlug) {
  const m = INTAKE_TRUSTED_LINE.exec(lastMeaningfulLine(c.body));
  if (!m)
    return null;
  const trusted = (typeof trustedSlug === "string" ? normalizeBotLogin(trustedSlug) : "") || INTENT_GATE_AUDIT_AUTHOR_SLUG;
  const byApp = trusted !== "" && c.authorIsBot === true && normalizeBotLogin(c.authorLogin) === trusted;
  const byTrusted = TRUSTED_AUTHOR_ASSOCIATIONS.has((c.authorAssociation ?? "").trim().toUpperCase());
  if (!byApp && !byTrusted)
    return null;
  return { reporter: m[1], permission: m[2] };
}
...
var init_pr_state = __esm(() => {
  ...
  INTAKE_TRUSTED_MARKER = SHIPFLOW_CONTRACT.markers.intakeTrusted;
  INTAKE_TRUSTED_LINE = new RegExp(`^${INTAKE_TRUSTED_MARKER.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")} reporter=(\\S+) permission=(\\S+) -->$`);
...
function ghIntakeTrustedAttestation(repo, number) {
  try {
    const trusted = resolveIntentGateAuditAuthorSlug();
    for (const c of ghIntentGateAuditCandidates(repo, number)) {
      const a = parseIntakeTrustedAttestation(c, trusted.slug);
      if (a)
        return a;
    }
    return null;
...
function decideIntakeGate(issue, ctx) {
  ...
  if (ctx.armedBefore === false && ctx.reporterTrusted === true)
    return "none";
  if (issue.associationLookupFailed)
  ...
  issue.command("next").description("Pick & claim the next open, unclaimed issue (for the work loop); exits 4 when none remain").option("--repo <fullname>", "Override target repo").option("--label <label>", "Only consider issues with this label").option("--assignee <login>", "Only consider issues assigned to this user (@me = the gh login; default under pickup-scope=assigned)").option("--agent <name>", "Agent label recorded on the claim (default: $SHIPFLOW_AGENT or hostname)").option("--ttl <minutes>", "Claim lifetime in minutes (default 120)").option("--json", "Output JSON").option("--yaml", "Output YAML").action(runAction(async (opts) => {
    ...
    for (const i of intakeMode === "off" ? [] : open) {
      ...
      if (action === "check-marker") {
        ...
        if (armedBefore === false) {
          const att = ghIntakeTrustedAttestation(repo, i.number);
          if (att) {
            reporterTrusted = true;
            console.warn(`\uD83E\uDEAA #${i.number}: filed via ShipFlow for @${att.reporter} (${att.permission} on ${repo}) — intake gate not armed.`);
          }
        }
        action = armedBefore === null ? "gate-this-pass" : decideIntakeGate(i, { intakeMode, armedBefore, reporterTrusted });
